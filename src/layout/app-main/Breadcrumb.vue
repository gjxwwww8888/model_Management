...
const levelList = ref()
const { settings, allRoutes} = useBasicStore()
...
const getBreadcrumb = () => {
  ...
  if (!isHasDashboard) {
    matched = [{ path: '/dashboard', redirect: '/index',  meta: { title: '首页' } }].concat(matched)
  }
  //过滤面包屑显示的数组
...
const handleLink = (item) => {
  ...
  //设置面包屑的切换功能没有redirect的情况
  let paths: any
  allRoutes.forEach((keyItem: any)=>{
    if (keyItem.path == path) {
        console.log(keyItem.path)
        let children = keyItem.children
        paths = path +'/'+ children[0].path
    }
  })

  if (path) router.push(pathCompile(paths));
}
//监听路由路径刷新 面包屑显示数组
watch(
  () => route.path,
